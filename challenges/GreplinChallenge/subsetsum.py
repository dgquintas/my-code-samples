...
def checkForValue(L, v):
    ...
    # complexity in O(n^2 lg(n)), as derived from
    # the O(n lg n) of the search over an ordered list, 
    # the copy of L and the T(n-1) nature of the recursion
    Li = list(L)
    res = []
    while len(Li):
        resi = []
...
def f(L):
    ...
    #elements
    for v in L[:2:-1]:
        # first two elements can't be the sum of anything else in the array if
        # elements are unique
        Li = list(L)
        Li.remove(v)
...
L = (3, 4, 9, 14, 15, 19, 28, 37, 47, 50, 54, 56, 59, 61, 70, 73, 78, 81, 92,
        95, 97, 99)
